  ...
  <body>
    ...
    if(isset($_POST['sendSubmit']))
    {
      ...
      move_uploaded_file($_FILES['eventImg']['tmp_name'], $dossier . $fichier); 
        ...
        $query = "INSERT INTO `event` (`id`, `nom_event`, `date_event`, `resume_event`, `prix_event`, `img_event`) VALUES (NULL, '".$_POST['eventName']."', '".$_POST['eventDate']."', \"".$_POST['eventSum']."\", '".$_POST['eventPrice']."', '".$_FILES['eventImg']['name']."');";
        
        $DB->insert($query);?>
        <h2>Evenement Ajouté!</h2>
        <?php
    }
    ?>
            ...
            <div class="regisFrm">
              <form enctype="multipart/form-data" action="addevent.php" method="post">
                <div class="form-group">
                  ...
                  <input class="form-control" type="text" name="eventName" placeholder="Donnez le nom de l'événement" required="">
                </div>
                <div class="form-group">
                  <label for="text">Date:</label>
                  <input class="form-control" type="text" name="eventDate" placeholder="AAAA-MM-JJ" required="" pattern="^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$">
                </div>
                <div class="form-group">
                  <label for="comment">Description:</label>
                  <textarea class="form-control" rows="10" id="comment" name=eventSum placeholder="Decrivez votre événement"></textarea>
                </div>
                <div class="form-group">
                  <label for="comment">Prix:</label>
                  <input class="form-control" type="text" name="eventPrice" placeholder="Fixez un prix (0 = gratuit)" required="">
                </div>
                <div class="form-group">
                  <label for="file">Image:</label>
                  <input class="form-control" type="file" name="eventImg" accept="image/png, image/jpeg, image/gif">
                </div>

                <button type="submit" name="sendSubmit" class="btn btn-default">Envoyer</button>
              </form>
            </div>
          </div>
